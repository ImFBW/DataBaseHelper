...
using DBH.Core.MyAttribute;
using DBH.Core.MyResolver;
...
namespace DBH.Core.Dapper
{
    ...
    public static partial class DapperHelper
    {
        ...
        private static DBCategory _dialect = DBCategory.SqlServer;
        ...
        public static void SetDialect(DBCategory dialect)
        {
            switch (dialect)
            {
                case DBCategory.MySql:
                    _dialect = DBCategory.MySql;
                    _encapsulation = "`{0}`";
                    _getIdentitySql = string.Format("SELECT LAST_INSERT_ID() AS id");
                    _getPagedListSql = "Select {SelectColumns} from {TableName} {WhereClause} Order By {OrderBy} LIMIT {Offset},{RowsPerPage}";
                    break;
                default:
                    _dialect = DBCategory.SqlServer;
                    _encapsulation = "[{0}]";
        ...
        private static bool IsEditable(PropertyInfo pi)
        {
            ...
            return write != null && (bool)write.AllowEdit;
        }
