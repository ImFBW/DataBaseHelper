...
(function () {
    ...
    var global = this
        ...
        , Split = function (ids, options) {
            ...
            if (!options.snapOffset && options.snapOffset != 0) options.snapOffset = 30
            ...
            var startDragging = function (e) {
                ...
                , drag = function (e) {
                    ...
                    //console.log("offset:" + offset + ",this.aMin：" + this.aMin + ",options.snapOffset：" + options.snapOffset + ",this.bMin:" + this.bMin + ",this.size:" + this.size);
                ...
                , calculateSizes = function () {
                    ...
                    //console.log("parentSize:" + parentSize + "    size：" + this.size + "   start：" + this.start);
                }
                , adjust = function (offset) {
                    // A size is the same as offset. B size is total size - A size.
                    // Both sizes are calculated from the initial parent percentage.
                    var a_width = ((offset + this.aGutterSize) / this.size * this.percentage);
                    var b_width = (this.percentage - ((offset + this.bGutterSize) / this.size * this.percentage));
                    //console.log("a_width:" + a_width + ",b_width:" + b_width);
                    //console.log("offset:" + offset + ",this.size:" + this.size + ",this.percentage:" + this.percentage + ",Left width：" + a_width + "%-" + this.aGutterSize + "px   Right width：" + b_width + "%-" + this.bGutterSize + "px");
                    //计算到0了，就不再计算另一个，否则会使宽度超出范围，导致页面错乱。
                    //console.log((a_width / 100) * this.size - this.aGutterSize);
                    //return;
                    if (a_width <= 0) {
                        //this.a.style[dimension] = 0;
                        //return;
                    }
                    this.a.style[dimension] = calc + '(' + a_width + '% - ' + this.aGutterSize + 'px)';
                    this.b.style[dimension] = calc + '(' + b_width + '% - ' +( this.bGutterSize + this.aGutterSize) + 'px)';
                }
                , fitMin = function () {
                    var self = this
                        , a = self.a
